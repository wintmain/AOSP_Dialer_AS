...
package com.wintmain.dialer.lookup.yellowpages;
...
public class YellowPagesApi {
    ...
    private String[] parseNameWebsiteUnitedStates() {
        ...
        if (m.find()) {
            website = Objects.requireNonNull(m.group(1)).trim();
            name = Objects.requireNonNull(m.group(2)).trim();
        }
    ...
    private String[] parseNameWebsiteCanada() {
        ...
        if (m.find()) {
            website = Objects.requireNonNull(m.group(1)).trim();
            name = LookupUtils.fromHtml(Objects.requireNonNull(m.group(2)).trim());
        }
