...
package com.wintmain.dialer.blocking;
...
public final class Blocking {
     ...
     *                   available. If {@code null} and {@code number} does not have a country
     *                   code, only the
        ...
        return DialerExecutorComponent.get(context)
                ...
                .submit(
                        () -> {
                            ...
                            for (String number : numbers) {
                                Selection selection =
                                        Selection.column(BlockedNumbers.COLUMN_ORIGINAL_NUMBER).is(
                                                "=", number);
                                String e164Number = PhoneNumberUtils.formatNumberToE164(number,
                                        countryIso);
                                if (e164Number != null) {
                                ...
                                operations.add(
                                        ContentProviderOperation.newDelete(
                                                        BlockedNumbers.CONTENT_URI)
                                                .withSelection(selection.getSelection(),
                            ...
                            Selection selection =
                                    Selection.builder()
                                            .or(Selection.column(
                                                    ...
                                                    .in(numbers))
                                            .or(Selection.column(BlockedNumbers.COLUMN_E164_NUMBER)
                                                    .in(e164Numbers))
                                            .build();
     ...
     *
     * <p>Blocking is only allowed on the primary user (the first user added). Primary user
     * cannot be
     * easily checked because {@link
     * android.provider.BlockedNumberContract#canCurrentUserBlockNumbers(Context)} is a slow IPC,
     * and
