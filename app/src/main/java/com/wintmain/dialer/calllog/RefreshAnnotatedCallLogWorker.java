...
import com.wintmain.dialer.calllog.datasources.CallLogMutations;
...
import com.wintmain.dialer.common.concurrent.DialerFutures;
import com.wintmain.dialer.inject.ApplicationContext;
import com.wintmain.dialer.metrics.FutureTimer;
import com.wintmain.dialer.metrics.FutureTimer.LogCatMode;
...
public class RefreshAnnotatedCallLogWorker {
    ...
    private ListenableFuture<RefreshResult> checkDirtyAndRebuildIfNecessary(boolean checkDirty) {
        ListenableFuture<Boolean> forceRebuildFuture =
                backgroundExecutorService.submit(
                        () -> {
                            ...
                            // Default to true. If the pref doesn't exist, the annotated call log
                            // hasn't been
                            // created and we just skip isDirty checks and force a rebuild.
                            boolean forceRebuildPrefValue =
                                    sharedPreferences.getBoolean(SharedPrefKeys.FORCE_REBUILD,
                                            true);
                            if (forceRebuildPrefValue) {
                                LogUtil.i(
                                        "RefreshAnnotatedCallLogWorker"
                                                + ".checkDirtyAndRebuildIfNecessary",
                                        "annotated call log has been marked dirty or does not "
                                                + "exist");
                            }
        ...
        return Futures.transformAsync(
                ...
                isDirty -> {
                    ...
                    if (isDirty) {
                        return Futures.transformAsync(
                                callLogState.isBuilt(), this::rebuild,
    ...
    private ListenableFuture<Boolean> isDirty() {
        ...
        // Simultaneously invokes isDirty on all data sources, returning as soon as one returns
        // true.
        ListenableFuture<Boolean> isDirtyFuture =
    ...
    private ListenableFuture<RefreshResult> rebuild(boolean isBuilt) {
        ...
        for (CallLogDataSource dataSource : dataSources.getDataSourcesExcludingSystemCallLog()) {
            fillFuture =
                    Futures.transformAsync(
                            fillFuture,
                            unused -> {
                                ListenableFuture<Void> dataSourceFuture = dataSource.fill(
                                        mutations);
        ...
        // After all data sources are filled, apply mutations (at this point "fillFuture" is the
        // result
        ...
        ListenableFuture<Void> applyMutationsFuture =
                Futures.transformAsync(
                        ...
                        unused -> {
                            ...
                            futureTimer.applyTiming(mutationApplierFuture,
                                    eventNameForApplyMutations(isBuilt));
        ...
        ListenableFuture<List<Void>> onSuccessfulFillFuture =
                Futures.transformAsync(
                        applyMutationsFuture,
                        unused -> {
                            List<ListenableFuture<Void>> onSuccessfulFillFutures =
                                    new ArrayList<>();
                            for (CallLogDataSource dataSource :
                                    dataSources.getDataSourcesIncludingSystemCallLog()) {
                                ListenableFuture<Void> dataSourceFuture =
                                        dataSource.onSuccessfulFill();
                                onSuccessfulFillFutures.add(dataSourceFuture);
                                String eventName = eventNameForOnSuccessfulFill(dataSource,
                                        isBuilt);
                                futureTimer.applyTiming(dataSourceFuture, eventName);
                            }
                            ListenableFuture<List<Void>> allFutures = Futures.allAsList(
                                    onSuccessfulFillFutures);
                            futureTimer.applyTiming(allFutures,
                                    eventNameForOverallOnSuccessfulFill(isBuilt));
                            return allFutures;
                        },
                        lightweightExecutorService);
        ...
        return Futures.transform(
                ...
                unused -> {
                    sharedPreferences.edit().putBoolean(SharedPrefKeys.FORCE_REBUILD, false)
                            .apply();
