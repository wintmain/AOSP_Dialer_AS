...
package com.wintmain.dialer.phonelookup.cp2;
...
public final class Cp2DefaultDirectoryPhoneLookup implements PhoneLookup<Cp2Info> {
    ...
    /**
     * Adds the {@code cp2ContactInfo} to the entries for all specified {@code dialerPhoneNumbers
     * } in
     * the {@code map}.
     */
    ...
    private Cp2Info lookupInternal(DialerPhoneNumber dialerPhoneNumber) {
        ...
        Cursor cursor = null;
        try {
            // Note: It would make sense to use PHONE_LOOKUP for valid numbers as well, but we
            // use PHONE
            ...
            if (!partitionedNumbers.validE164Numbers().isEmpty()) {
                cursor =
                        queryPhoneTableBasedOnE164(
                                Cp2Projections.getProjectionForPhoneTable(),
                                partitionedNumbers.validE164Numbers());
            } else {
                cursor =
                        queryPhoneLookup(
    ...
    public ListenableFuture<Boolean> isDirty(ImmutableSet<DialerPhoneNumber> phoneNumbers) {
        if (!PermissionsUtil.hasContactsReadPermissions(appContext)) {
            ...
            Predicate<PhoneLookupInfo> phoneLookupInfoIsDirtyFn =
                    phoneLookupInfo ->
                            !phoneLookupInfo.getDefaultCp2Info().equals(
        ...
        if (partitionedNumbers.invalidNumbers().size() > getMaxSupportedInvalidNumbers()) {
            // If there are N invalid numbers, we can't determine determine dirtiness without
            // running N
            // queries; since running this many queries is not feasible for the (lightweight)
            // isDirty
            // check, simply return true. The expectation is that this should rarely be the case
            // as the
        ...
        return Futures.transformAsync(
                lastModifiedFuture,
                lastModified -> {
                    // We are always going to need to do this check and it is pretty cheap so do
                    // it first.
                    ...
                    return Futures.transformAsync(
                            ...
                            anyContactsDeleted -> {
                                ...
                                // Hopefully the most common case is there are no contacts
                                // updated; we can detect
                                // this cheaply.
                                ...
                                return Futures.transformAsync(
                                        ...
                                        noContactsModifiedSince -> {
                                            if (noContactsModifiedSince) {
                                                LogUtil.v(
                                                        "Cp2DefaultDirectoryPhoneLookup.isDirty",
                                                        "returning false because no contacts "
                                                                + "modified since last run");
                                                return Futures.immediateFuture(false);
                                            }
                                            // This method is more expensive but is probably the
                                            // most likely scenario; we
                                            // are looking for changes to contacts which have
                                            // been called.
                                            ...
                                            ListenableFuture<Boolean> contactsUpdatedFuture =
                                                    Futures.transformAsync(
                                                            contactIdsFuture,
                                                            contactIds -> contactsUpdated(
                                                                    contactIds, lastModified),
                                            ...
                                            return Futures.transformAsync(
                                                    ...
                                                    contactsUpdated -> {
                                                        if (contactsUpdated) {
                                                            LogUtil.v(
                                                                    "Cp2DefaultDirectoryPhoneLookup.isDirty",
                                                                    "returning true because a "
                                                                            + "previously called "
                                                                            + "contact was "
                                                                            + "updated");
                                                            return Futures.immediateFuture(true);
                                                        }
                                                        // This is the most expensive method so
                                                        // do it last; the scenario is that
                                                        // a contact which has been called got
                                                        // disassociated with a number and
                                                        // we need to clear their information.
                                                        ListenableFuture<Set<Long>>
    ...
    /**
     * Returns set of contact ids that correspond to {@code dialerPhoneNumbers} if the contact
     * exists.
     */
    private ListenableFuture<Set<Long>> queryPhoneTableForContactIds(
        ...
        // Then run a separate query for each invalid number. Separate queries are done to
        // accomplish
                        ...
                        if (cursor.moveToFirst()) {
                            ...
                            do {
                                ...
                                for (Cp2ContactInfo info :
                                        phoneLookupInfo.getDefaultCp2Info()
    ...
    private ListenableFuture<Set<Long>> queryPhoneTableForContactIdsBasedOnE164(
        ...
        return backgroundExecutorService.submit(
                () -> {
                    ...
                    try (Cursor cursor =
                                 queryPhoneTableBasedOnE164(new String[]{Phone.CONTACT_ID},
                                         validE164Numbers)) {
                        if (cursor == null) {
                            LogUtil.w(
                                    "Cp2DefaultDirectoryPhoneLookup"
                                            + ".queryPhoneTableForContactIdsBasedOnE164",
                                    "null cursor");
                            return contactIds;
                        }
                        while (cursor.moveToNext()) {
        ...
        return backgroundExecutorService.submit(
                () -> {
                    try (Cursor cursor =
                        ...
                        if (cursor == null) {
                            LogUtil.w("Cp2DefaultDirectoryPhoneLookup.noContactsModifiedSince",
                                    "null cursor");
                            return false;
                        }
                        return cursor.getCount() == 0;
                                 ...
                                 appContext
                                         ...
                                         .query(
                                                 ...
                                                 DeletedContacts.CONTACT_DELETED_TIMESTAMP
                                                         + " limit 1")) {
                        if (cursor == null) {
                            LogUtil.w("Cp2DefaultDirectoryPhoneLookup.anyContactsDeletedSince",
                                    "null cursor");
                            return false;
                        }
                        return cursor.getCount() > 0;
    ...
    public ListenableFuture<ImmutableMap<DialerPhoneNumber, Cp2Info>> getMostRecentInfo(
        ...
        return Futures.transformAsync(
                lastModifiedFuture,
                lastModified -> {
                    // Build a set of each DialerPhoneNumber that was associated with a contact,
                    // and is no
                    ...
                    return Futures.transformAsync(
                            ...
                            deletedPhoneNumbers -> {
                                ...
                                // If there are too many invalid numbers, just defer the work to
                                // render time.
                                ArraySet<DialerPhoneNumber> unprocessableNumbers =
                                        findUnprocessableNumbers(existingInfoMap);
                                Map<DialerPhoneNumber, Cp2Info> existingInfoMapToProcess =
                                        existingInfoMap;
                                if (!unprocessableNumbers.isEmpty()) {
                                    existingInfoMapToProcess =
                                            Maps.filterKeys(
                                                    existingInfoMap,
                                                    number -> !unprocessableNumbers.contains(
                                                            number));
                                }

                                // For each DialerPhoneNumber that was associated with a contact
                                // or added to a
                                // contact, build a map of those DialerPhoneNumbers to a set
                                // Cp2ContactInfos, where
                                ...
                                ListenableFuture<Map<DialerPhoneNumber, Set<Cp2ContactInfo>>>
                                        ...
                                        buildMapForUpdatedOrAddedContacts(
                                                existingInfoMapToProcess, lastModified,
                                                deletedPhoneNumbers);
                                ...
                                return Futures.transform(
                                        ...
                                        updatedContacts -> {
                                            ...
                                            // Start build a new map of updated info. This will
                                            // replace existing info.
                                            ImmutableMap.Builder<DialerPhoneNumber, Cp2Info>
                                                    newInfoMapBuilder =
                                                    ImmutableMap.builder();

                                            // For each DialerPhoneNumber in existing info...
                                            for (Entry<DialerPhoneNumber, Cp2Info> entry :
                                                    existingInfoMap.entrySet()) {
                                                ...
                                                // If the contact was updated, replace the
                                                // Cp2ContactInfo list
                                                if (updatedContacts.containsKey(
                                                        dialerPhoneNumber)) {
                                                    infoBuilder
                                                            .clear()
                                                            .addAllCp2ContactInfo(
                                                                    updatedContacts.get(
                                                                            dialerPhoneNumber));
                                                    // If it was deleted and not added to a new
                                                    // contact, clear all the CP2
                                                    // information.
                                                } else if (deletedPhoneNumbers.contains(
                                                        dialerPhoneNumber)) {
                                                    infoBuilder.clear();
                                                } else if (unprocessableNumbers.contains(
                                                        dialerPhoneNumber)) {
                                                    // Don't ever set the "incomplete" bit for
                                                    // numbers which are empty; this
                                                    // causes unnecessary render time work
                                                    // because there will never be contact
                                                    // information for an empty number. It is
                                                    // also required to pass the
                                                    // assertion check in the new voicemail
                                                    // fragment, which verifies that no
                                                    // voicemails rows are considered
                                                    // "incomplete" (the voicemail fragment
                                                    // does not have the ability to fetch
                                                    // information at render time).
                                                    if (!dialerPhoneNumber.getNormalizedNumber()
                                                            .isEmpty()) {
                                                        // Don't clear the existing info when the
                                                        // number is unprocessable. It's
                                                        // likely that the existing info is
                                                        // up-to-date so keep it in place so
                                                        // that the UI doesn't pop when the query
                                                        // is completed at display time.
                                                        infoBuilder.setIsIncomplete(true);
                                                    }
                                                }

                                                // If the DialerPhoneNumber didn't change, add
                                                // the unchanged existing info.
                                                newInfoMapBuilder.put(dialerPhoneNumber,
                                                        infoBuilder.build());
                                            }
        ...
        return backgroundExecutorService.submit(
                () -> {
                    if (currentLastTimestampProcessed != null) {
                        sharedPreferences
                                .edit()
                                .putLong(PREF_LAST_TIMESTAMP_PROCESSED,
        ...
        return backgroundExecutorService.submit(
                () -> {
                    ...
                    for (Entry<DialerPhoneNumber, Cp2Info> entry : existingInfoMap.entrySet()) {
                        ...
                        // If the number was deleted, we need to check if it was added to a new
                        // contact.
                        ...
                        // When the PhoneLookupHistory contains no information for a number,
                        // because for
                        // example the user just upgraded to the new UI, or cleared data, we need
                        // to check for
                        // updated info.
                        if (existingInfo.getCp2ContactInfoCount() == 0) {
                            ...
                            // Store the contact id if it exist, else automatically add the
                            // DialerPhoneNumber to
                            // our set of DialerPhoneNumbers we want to update.
                            for (Cp2ContactInfo cp2ContactInfo :
                                    existingInfo.getCp2ContactInfoList()) {
                                long existingContactId = cp2ContactInfo.getContactId();
                                if (existingContactId == 0) {
                                    // If the number doesn't have a contact id, for various
                                    // reasons, we need to look
                                    // up the number to check if any exists. The various reasons
                                    // this might happen
                                    // are:
                                    //  - An existing contact that wasn't in the call log is now
                                    //  in the call log.
                                    //  - A number was in the call log before but has now been
                                    //  added to a contact.
                                    //  - A number is in the call log, but isn't associated with
                                    //  any contact.
                                    updatedNumbers.add(dialerPhoneNumber);
                                } else {
                    ...
                    // Contacts.CONTACT_LAST_UPDATED_TIMESTAMP is after lastModified, such that
                    // Contacts._ID
                    // is in our set of contact IDs we build above.
                    if (!contactIds.isEmpty()) {
                        try (Cursor cursor = queryContactsTableForContacts(contactIds,
                                lastModified)) {
                            int contactIdIndex = cursor.getColumnIndex(Contacts._ID);
                            int lastUpdatedIndex = cursor.getColumnIndex(
                                    Contacts.CONTACT_LAST_UPDATED_TIMESTAMP);
                            cursor.moveToPosition(-1);
                            while (cursor.moveToNext()) {
                                // Find the DialerPhoneNumber for each contact id and add it to
                                // our updated numbers
                                // set. These, along with our number not associated with any
                                // Cp2ContactInfo need to
                                // be updated.
                                long contactId = cursor.getLong(contactIdIndex);
                                updatedNumbers.addAll(
                                        findDialerPhoneNumbersContainingContactId(existingInfoMap,
                                                contactId));
    ...
    /**
     * 1. get all contact ids. if the id is unset, add the number to the list of contacts to look
     * up.
     * 2. reduce our list of contact ids to those that were updated after lastModified. 3. Now we
     * have
    ...
    buildMapForUpdatedOrAddedContacts(
        ...
        return Futures.transformAsync(
                ...
                updatedNumbers -> {
                    ...
                    // Divide the numbers into those that are valid and those that are not. Issue
                    // a single
                    // batch query for the valid numbers against the PHONE table, and in parallel
                    // issue
                    // individual queries against PHONE_LOOKUP for each invalid number.
                    // TODO(zachh): These queries are inefficient without a lastModified column
                    //  to filter on.
                    PartitionedNumbers partitionedNumbers =
                    ...
                    List<ListenableFuture<Set<Cp2ContactInfo>>> invalidNumbersFuturesList =
                            new ArrayList<>();
                    for (String invalidNumber : partitionedNumbers.invalidNumbers()) {
                        invalidNumbersFuturesList.add(
                                individualQueryForInvalidNumber(invalidNumber));
                    }
                    ...
                    Callable<Map<DialerPhoneNumber, Set<Cp2ContactInfo>>> computeMap =
                            () -> {
                                // These get() calls are safe because we are using whenAllSucceed
                                // below.
                                Map<String, Set<Cp2ContactInfo>> validNumbersResult =
                                        validNumbersFuture.get();
                                List<Set<Cp2ContactInfo>> invalidNumbersResult =
                                        invalidNumbersFuture.get();

                                Map<DialerPhoneNumber, Set<Cp2ContactInfo>> map = new ArrayMap<>();

                                // First update the map with the valid number results.
                                for (Entry<String, Set<Cp2ContactInfo>> entry :
                                        validNumbersResult.entrySet()) {
                                    String validNumber = entry.getKey();
                                    Set<Cp2ContactInfo> cp2ContactInfos = entry.getValue();

                                    Set<DialerPhoneNumber> dialerPhoneNumbers =
                                            partitionedNumbers.dialerPhoneNumbersForValidE164(
                                                    validNumber);

                                    addInfo(map, dialerPhoneNumbers, cp2ContactInfos);

                                    // We are going to remove the numbers that we've handled so
                                    // that we later can
                                    // detect numbers that weren't handled and therefore need to
                                    // have their contact
                                    // information removed.
                                ...
                                // The leftovers in updatedNumbers that weren't removed are
                                // numbers that were
                                // previously associated with contacts, but are no longer. Remove
                                // the contact
                ...
                () -> {
                    ...
                    try (Cursor cursor =
                                 queryPhoneTableBasedOnE164(
                                         ...
                                         validE164Numbers)) {
                        if (cursor == null) {
                            LogUtil.w("Cp2DefaultDirectoryPhoneLookup.batchQueryForValidNumbers",
                                    "null cursor");
                        } else {
                            while (cursor.moveToNext()) {
                                String validE164Number =
                                        Cp2Projections.getNormalizedNumberFromCursor(cursor);
                                Set<Cp2ContactInfo> cp2ContactInfos = cp2ContactInfosByNumber.get(
                                        validE164Number);
    ...
    private ListenableFuture<Set<Cp2ContactInfo>> individualQueryForInvalidNumber(
        ...
        return backgroundExecutorService.submit(
                () -> {
                    ...
                    try (Cursor cursor =
                                 queryPhoneLookup(Cp2Projections.getProjectionForPhoneLookupTable(),
                                         invalidNumber)) {
                        if (cursor == null) {
                            LogUtil.w(
                                    "Cp2DefaultDirectoryPhoneLookup"
                                            + ".individualQueryForInvalidNumber",
                                    "null cursor");
                        } else {
                            while (cursor.moveToNext()) {
                                cp2ContactInfos.add(
    ...
    private ListenableFuture<Set<DialerPhoneNumber>> getDeletedPhoneNumbers(
        ...
        return backgroundExecutorService.submit(
                () -> {
                    // Build set of all contact IDs from our existing data. We're going to use
                    // this set to
                    ...
                    // Start building a set of DialerPhoneNumbers that were associated with now
                    // deleted
                    // contacts.
                    try (Cursor cursor = queryDeletedContacts(contactIds, lastModified)) {
                        // We now have a cursor/list of contact IDs that were associated with
                        // deleted contacts.
    ...
    private Set<DialerPhoneNumber> findDeletedPhoneNumbersIn(
        ...
        int deletedTimeIndex = cursor.getColumnIndexOrThrow(
                DeletedContacts.CONTACT_DELETED_TIMESTAMP);
    ...
    /**
     * We cannot efficiently process invalid numbers because batch queries cannot be constructed
     * which
     * accomplish the necessary loose matching. We'll attempt to process a limited number of
     * them, but
