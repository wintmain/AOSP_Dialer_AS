...
import com.wintmain.dialer.databasepopulator.BlockedBumberPopulator;
import com.wintmain.dialer.databasepopulator.CallLogPopulator;
import com.wintmain.dialer.databasepopulator.ContactsPopulator;
import com.wintmain.dialer.persistentlog.PersistentLogger;
...
//      VoicemailPopulator.populateVoicemail(input.context, input.fastMode);
    ...
    private static class PopulateDatabaseWorker
            implements Worker<PopulateDatabaseWorkerInput, Void> {
        @Nullable
        @Override
        public Void doInBackground(PopulateDatabaseWorkerInput input) {
...
//      VoicemailPopulator.deleteAllVoicemail(context);
    ...
    private static class ClearPreferredSimWorker implements Worker<Context, Void> {
        ...
        @Override
        public Void doInBackground(Context context) {
            context.getContentResolver().delete(PreferredSimFallbackContract.CONTENT_URI, null,
                    null);
            return null;
        }
    }
