            ...
            switch (msg.what) {
                ...
                case SEND_MESSAGE:
                    ...
                    if (Constants.BUBBLE_BREAKER.equals(message)) {
                        ...
                        sendMessageDelayed(
                                obtainMessage(SEND_MESSAGE, nextTyping()),
                                1000 * (1 + random.nextInt(10)));
                    } else {
                        // Wait up to 2s between typing.
                        sendMessageDelayed(obtainMessage(SEND_MESSAGE, nextTyping()),
                                200 * random.nextInt(10));
                    }
                    break;
                default: // fall out
            }
        ...
        private String nextTyping() {
            ...
            if (currentTypingPosition < currentTypingMessage.length()) {
                int size = random.nextInt(
                        currentTypingMessage.length() - currentTypingPosition + 1);
                String messageToType =
                        currentTypingMessage.substring(currentTypingPosition,
                                currentTypingPosition + size);
