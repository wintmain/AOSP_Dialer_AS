...
package com.wintmain.dialer.simulator.service;
...
import com.wintmain.dialer.simulator.impl.SimulatorMainPortal;
            ...
            new ISimulatorService.Stub() {
                ...
                public void makeIncomingCall(String callerId, int presentation) {
                    doSecurityCheck(
                            () -> {
                                ...
                                simulatorMainPortal.execute(
                                        new String[]{VOICECALL, CUSTOMIZED_INCOMING_CALL});
                            });
                }
                ...
                public void makeOutgoingCall(String callerId, int presentation) {
                    doSecurityCheck(
                            () -> {
                                ...
                                simulatorMainPortal.setPresentation(presentation);
                                simulatorMainPortal.execute(
                                        new String[]{VOICECALL, CUSTOMIZED_OUTGOING_CALL});
                            });
                }
                ...
                public void makeIncomingEnrichedCall() throws RemoteException {
                    doSecurityCheck(
                            () -> {
                                simulatorMainPortal.execute(
                                        new String[]{VOICECALL, INCOMING_ENRICHED_CALL});
                            });
                }
                ...
                public void makeOutgoingEnrichedCall() throws RemoteException {
                    doSecurityCheck(
                            () -> {
                                simulatorMainPortal.execute(
                                        new String[]{VOICECALL, OUTGOING_ENRICHED_CALL});
                            });
                }
                ...
                public void populateMissedCall(int num) throws RemoteException {
                    doSecurityCheck(
                            () -> {
                                ...
                                simulatorMainPortal.execute(
                                        new String[]{NOTIFICATIONS, MISSED_CALL});
                            });
                }
                ...
                private void doSecurityCheck(Runnable runnable) {
                    if (!hasAccessToService()) {
                        throw new RuntimeException(
                                "Client doesn't have access to Simulator service!");
                    }
                    runnable.run();
                }
            };
        ...
        if (packageName.isPresent()) {
            try {
                PackageInfo packageInfo =
                        getPackageManager().getPackageInfo(packageName.get(),
                                PackageManager.GET_SIGNATURES);
                MessageDigest messageDigest = MessageDigest.getInstance("MD5");
                if (packageInfo.signatures.length != 1) {
                    throw new NotOnlyOneSignatureException(
                            "The client has more than one signature!");
                }
                Signature signature = packageInfo.signatures[0];
                return isCertificateValid(messageDigest.digest(signature.toByteArray()),
                        this.certificates);
            } catch (NameNotFoundException | NoSuchAlgorithmException |
                     NotOnlyOneSignatureException e) {
